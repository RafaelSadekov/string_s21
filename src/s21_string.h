...
void *s21_memcpy(void *dest, const void *src,
                 size_t n);  // 3 Скопировать блок данных из памяти.
...
int s21_memcmp(const void *str1, const void *str2,
               size_t n);  // 2 Сравнение двух блоков памяти.
...
void *s21_memchr(const void *str, int c,
                 size_t count);  // 1 Поиск символа в блоке памяти
char *s21_strchr(const char *str,
                 int c);  // 6 strchr	Найти первое вхождение символа в строке.
...
// вхождения. 12 strpbrk	Выполняет поиск первого вхождения в строку str1
// любого из символов строки str2, и возвращает указатель на  найденный символ.
char *s21_strrchr(
    const char *str,
    int c);  // 13 strrchr	Поиск последнего вхождения указанного символа.
...
void *s21_memset(
    void *str, int c,
    size_t n);  // 4 Заполнить n байтов блока памяти указанным символом.
char *strerror(int errnum);  // 10 Интерпретация кодов ошибок в понятные
                             // сообщения об ошибках
size_t s21_strlen(const char *str);  // 11 Вычисляет длину строки str, не

...
#endif
/*
#define S21_MAX_BUFFER_SIZE 1 << 10 //в string.h
...
Использование static внутри функции является самым простым.
 Это просто означает, что после того, как переменная была инициализирована, 
 она остается в памяти до конца программы. Вы можете думать об этом, как о 
 переменной, которая хранит свое значение до полного завершения программы.
...
https://fossies.org/linux/glibc/sysdeps/gnu/errlist.h
https://codebrowser.dev/glibc/glibc/sysdeps/gnu/errlist.c.html#_sys_errlist_internal
